...
def marking(eta):
    ...
    rolling_sum = 0.
    for i, e in enumerate(eta.vetor.array[sorted_cells]):
        rolling_sum += e
...
def main():
    ...
    for i in max_step:
        ...
        u = primal(mesh)
        with XDMFFile(mesh.mpi_comm(), f"output/solutions/u_{str(i).zfill(4)}.xdmf", "w") as of:
            of.write_mesh(mesh)
            of.write_function(u)

        eta_h = estimate_primal(u)
        with XDMFFile(mesh.mpi_comm(), f"output/bw_estimators/eta_{str(i).zfill(4)}.xdmf", "w") as of:
            of.write_mesh(mesh)
            of.write_function(eta_h)

        markers_tag = marking(eta_h)

        mesh.topology.create_entities(mesh.topology.dim - 1)
        refined_mesh = dolfinx.mesh.refine(mesh, cell_markers=markers_tag)

        with XDMFFile(MPI.COMM_WORLD, f'./output/meshes/mesh_{str(i).zfill(4)}.xdmf', 'w') as fo:
            fo.write_mesh(refined_mesh)
