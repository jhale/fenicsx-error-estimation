...
def estimate_bw(u_h, f, parameters):
    print(f"BW {parameters[0]} {parameters[1]}")
...
def adaptive_refinement(estimator, bw_param=None):
    if estimator == "bw":
        estimator_path = f"bw_{str(bw_param[0])}_{str(bw_param[1])}"
    else:
        estimator_path = estimator

    try:
        if os.path.exists(path=os.path.join("output_minimal", estimator_path)):
            st.rmtree(os.path.join("output_minimal", estimator_path))
            os.makedirs(os.path.join("output_minimal", estimator_path))
        else:
            os.makedirs(os.path.join("output_minimal", estimator_path))
    ...
    for l in range(0, 20):
        ...
        with XDMFFile(MPI.COMM_WORLD, os.path.join("output_minimal", estimator_path, f"u_h_{str(l).zfill(4)}.xdmf"), "w") as fo:
            fo.write_mesh(mesh)
        ...
        if estimator == "bw":
            eta_u = estimate_bw(u_h, f, bw_param)

            eta_z = estimate_bw(z_h, weight, bw_param)

            eta_w = estimate_wgo(eta_u, eta_z)
        elif estimator == "res":
            eta_u = estimate_residual(u_h, f)

            eta_z = estimate_residual(z_h, weight)

            eta_w = estimate_wgo(eta_u, eta_z)
        elif estimator == "zz":
            eta_u = estimate_zz(u_h)

            eta_z = estimate_zz(z_h)

            eta_w = estimate_wgo(eta_u, eta_z)

        eta_u_global = np.sqrt(eta_u.vector.sum())
        eta_z_global = np.sqrt(eta_z.vector.sum())
        eta_w_global = eta_u_global * eta_z_global


        result[f'error_{estimator}_u'] = eta_u_global
        result[f'error_{estimator}_z'] = eta_z_global
        result[f'error_{estimator}_w'] = eta_w_global
    ...
    df.to_pickle(os.path.join("output_minimal", estimator_path, "results.pkl"))

adaptive_refinement("bw", bw_param=(3, 4))

'''
for i in [1, 2, 3]:
    for j in [1, 2, 3, 4][i:]:
        adaptive_refinement("bw", bw_param=(i, j))
'''
'''
adaptive_refinement("res")
adaptive_refinement("zz")
'''
