...
def main():
    ...
    try:
        with open(os.path.join(OUTPUT_DIR, 'times.pkl'), 'rb') as fi:
            times = pkl.load(fi)
    except:
        times = {'num processors': [1, 2, 4, 8, 16], 'dofs': [], 'primal solve': [], 'bw estimator': [], 'bw estimator D': [], 'residual estimator': [], 'zz estimator': []}
    ...
    print(f'BW + D EST. COMPUTATION...')
    with Timer() as t:
        eta_bw_D = estimate_bw(k, u, dirichlet_est=True)
        times['bw estimator D'].append(t.elapsed()[0])
    ...
    '''
    print(f'RESIDUAL EST. COMPUTATION...')
    with Timer() as t:
        eta_res = estimate_residual(k, u, dirichlet_osc=True)
        times['residual estimator'].append(t.elapsed()[0])
    '''

    '''
    if k == 1:
        print(f'ZZ EST. COMPUTATION...')
        with Timer() as t:
            eta_zz_D = estimate_zz(u, dirichlet_osc=True)
            times['zz estimator'].append(t.elapsed()[0])
    '''
